...
# This script uses the WISP monitor to read Vreg, the WISP regulated
...
def cleanup():
    ...
    mon.sendCmd(wispmon.USB_CMD_LOG_VREG_END) # stop logging Vreg
    ...
    print "\tVreg data points:\t%d" % numSamples
...
def main():
    ...
    mon.sendCmd(wispmon.USB_CMD_LOG_VREG_BEGIN) # start logging Vreg
    ...
    while(curCycles * mon.CLK_PERIOD < SAMPLE_TIME):
        ...
        while mon.buildRxPkt(buf):
            ...
            elif(mon.rxPkt.descriptor == wispmon.USB_RSP_VREG):
                # received Vreg data
                numSamples += 1
                Vreg = bytearray([wispmon.USB_RSP_VREG,
                                  mon.rxPkt.data[1], mon.rxPkt.data[0]])
                
                # write both values now to ensure that we don't get a time
                # with no corresponding Vreg value
                fp.write(time + Vreg)
