...
# This script uses the WISP monitor to read Vcap and Vrect as fast as the
# monitor can for some time period.  The time values
...
def cleanup():
    global curCycles, VcapSamples, VrectSamples
    
    mon.sendCmd(wispmon.USB_CMD_LOG_VCAP_END) # stop logging Vcap
    mon.sendCmd(wispmon.USB_CMD_LOG_VRECT_END) # stop logging Vrect
    ...
    print "\tVrect data points:\t%d" % VrectSamples
...
def main():
    ...
    mon.sendCmd(wispmon.USB_CMD_LOG_VRECT_BEGIN) # start logging Vrect
    ...
    while(curCycles * mon.CLK_PERIOD < SAMPLE_TIME):
        ...
        if mon.buildRxPkt(buf):
            ...
            elif(mon.rxPkt.descriptor == wispmon.USB_RSP_VRECT):
                # received Vrect data
                VrectSamples += 1
                Vrect = bytearray([wispmon.USB_RSP_VRECT,
                                   mon.rxPkt.data[1], mon.rxPkt.data[0]])
