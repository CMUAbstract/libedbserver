...
# This script uses the WISP monitor to read Vrect, the WISP rectified
...
def cleanup():
    ...
    mon.sendCmd(wispmon.USB_CMD_LOG_VRECT_END) # stop logging Vcap
    ...
    print "\tVrect data points:\t%d" % numSamples
    ...
    while(curCycles * mon.CLK_PERIOD < SAMPLE_TIME):
        ...
        if mon.buildRxPkt(buf):
            ...
            if(mon.rxPkt.descriptor == wispmon.USB_RSP_TIME):
                ...
                # with no corresponding Vrect value
                fp.write(time + Vrect)
