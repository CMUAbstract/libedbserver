 ...
 * 					activity.  If the timer is used to time several events
 * 					at once, only the last call to Timer1_set will result
 * 					in a timer interrupt and call to the callback.
 ******************************************************************************/
...
{
	TA1CCR0 = smclk_cycles; // set timer compare register

	if(callbackFn != NULL) {
		pCallbackFn = callbackFn; // set callback function
